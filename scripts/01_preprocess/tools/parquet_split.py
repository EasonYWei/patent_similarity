"""Chunked conversion from raw Stata patent data to stock-range Parquet files."""
...
from .io import pandas_to_polars
...
def align_to_schema(df: pl.DataFrame, columns: list[str], schema: dict[str, pl.DataType]) -> pl.DataFrame:
    ...
    return df.select(
        [pl.col(column).cast(schema[column], strict=False).alias(column) for column in columns]
    )
...
def prepare_output_dir(output_dir: Path, overwrite: bool) -> Path:
    ...
    if (existing_final or existing_temp) and not overwrite:
        raise FileExistsError(
            f"{output_dir} already contains parquet output or temp parts. Use --overwrite to rebuild it."
        )
    if overwrite:
...
def write_temp_parts(input_path: Path, temp_dir: Path, chunk_size: int, compression: str) -> tuple[int, int, list[str]]:
    total_rows = 0
    invalid_rows = 0
    ...
    for chunk_index, pdf in enumerate(reader, start=1):
        ...
        if invalid_count:
            ...
            raise ValueError(
                f"Found {invalid_count} rows with invalid stock codes in chunk {chunk_index}: {examples}"
            )
...
def split_patents_dta_to_parquet(
    *,
    input_path: Path,
    output_dir: Path,
    chunk_size: int,
    ...
    cleanup_failed_temp: bool = False,
) -> int:
    ...
    temp_dir = prepare_output_dir(output_dir, overwrite)
    report: dict[str, Any] = {
    ...
    try:
        total_rows, invalid_rows, columns = write_temp_parts(input_path, temp_dir, chunk_size, compression)
        manifest = consolidate_all(temp_dir, output_dir, columns, compression)
        validate_outputs(output_dir, manifest, columns, total_rows)
        ...
        if report["deleted_source"]:
            logging.info("Deleted source after verification: %s", input_path)
        return 0
